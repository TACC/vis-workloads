...
match_reader = re.compile("(\w+)Reader::GatherInformation,  +(\d*.*\d+) +seconds")
...
def get_logs() :
    ...
    for component in components:
        ...
        if len(default_log_threshold) != 0:
           ...
           print "setting log threshold: " + str(default_log_threshold[str(component)])

        #timerInfo.SetLogThreshold(5) #Carson debug
        session.GatherInformation(component, timerInfo, 0)
        #print "timerinfo 0: " + timerInfo.GetLog(0) #Carson debug
        #print "timerinfo 1: " + timerInfo.GetLog(1) #Carson debug
...
def __parse_line (line) :
    ...
    match = match_reader.match(line)
    if match != None:
        found = True
        if showparse:
            print "READER:", cnt, line
        name = match.group(1) + "Reader"
        id = name
    ...
    for i in logs:
            ...
            for cnt in xrange(start_frame, len(frames)):
                ...
                for record in frames[cnt]:
                    if 'id' in record:
                        ...
                        if record['name'] == "Still" or record['name'] == "OpenGL Dev":
                          print "found still record"
                          while len(parse_results) <= cnt:
                            parse_results.append([])
                          while len(parse_results[cnt-1]) <= rank:
                            parse_results[cnt-1].append([])
                          if  not  parse_results[cnt-1][rank]:
                            parse_results[cnt-1][rank] = dict()
                          if parse_results[cnt-1][rank].get(record['name']) == None:
                            parse_results[cnt-1][rank][record['name']] = float(record['duration'])
                          else:
                            parse_results[cnt-1][rank][record['name']] += float(record['duration'])
                            #print "adding frame results for proc " + str(rank) + " " + str(cnt) + " " + str(record['duration'])
        #print
        #print
        ...
        if not tabular:
            ...
            for x in filters:
                ...
                while len(parse_results) < cnt:
                  parse_results.append([])
                while len(parse_results[0]) <= rank:
                  parse_results[0].append([])
                if not parse_results[0][rank]:
                  parse_results[0][rank] = dict()
                if parse_results[0][rank].get(record['name']) == None:
                  parse_results[0][rank][record['name']] = float(record['duration'])
                if record['name'].find("Reader") != -1:
                  print "found reader record"
                  if parse_results[0][rank].get('reader') == None:
                    parse_results[0][rank]['reader'] = 0.0
                  parse_results[0][rank]['reader'] += float(record['duration'])
                else:
